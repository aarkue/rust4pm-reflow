...
struct RegisterBindingAttrs {
    ...
    /// `returns_handle`: the return value is a `CustomRegistryValue`, stored in the registry and
    /// reported as an id, like a big-type return.
    returns_handle: bool,
}
...
struct ArgOptions {
    ...
    /// `#[bind(state)]`: not a JSON argument at all -- the binding is handed a read-only
    /// [`StateRef`] over the registry instead.
    is_state: bool,
        ...
        if attr.path().is_ident("bind") {
            attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("state") {
                    ...
                    return Err(meta.error(
                        "unknown #[bind] option, expected one of `state`, `handle`, \
...
/// One function argument, as the codegen below needs it.
///
/// A struct rather than a tuple: several passes match on the last element only
/// (`|(.., mut_name)|`), which would silently rebind if a field were appended.
struct ArgInfo {
    name: String,
    ...
    /// The referent type and its mutability, for a `#[bind(handle)]` argument.
    handle_elem: Option<(Type, bool)>,
}

impl ArgInfo {
    /// Whether this argument needs the write lock, like a `&mut` big type does.
...
pub fn register_binding(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let args_info: syn::parse::Result<Vec<ArgInfo>> = input_fn
        ...
        .map(|arg| match arg {
            FnArg::Typed(pat_type) => {
                ...
                // A handle is passed as an id and resolved through `FromContext`, exactly as a
                // big type is, so the reference has to survive into `extract_param`'s type
                // argument. The attribute also wins over the name-suffix test below, which is
                // what lets a downstream type whose name happens to end in `OCEL` be a handle.
    ...
    // `#[bind]` is not a real attribute to rustc, so it has to be stripped before `#input_fn` is
    // emitted on any path, including the error paths below.
    ...
    let has_any_mut_big_type = args_info
        .iter()
        .any(|a| a.mut_big_type_name.is_some() || a.is_mut_handle());

    // A `&mut` big-type argument is resolved with `__state_guard.get_mut(..)`, whose borrow is
    // live until the call. Lending the same guard out as a shared `StateRef` at the same time is
    // E0502. Say so here rather than emitting code that fails to borrow-check.
    if has_any_mut_big_type && args_info.iter().any(|a| a.opts.is_state) {
        let err = syn::Error::new_spanned(
            &input_fn.sig,
            "#[bind(state)] cannot be combined with a `&mut` big-type or `#[bind(handle)]` \
             argument. Return the value instead of taking it by `&mut`.",
        )
        .to_compile_error();
        return TokenStream::from(quote! { #input_fn #err });
    }

    // The write-lock path extracts every other argument from JSON, and a reference deserializes
    // from nothing.
    if has_any_mut_big_type {
        if let Some(shared) = args_info.iter().find(|a| {
            ...
            let err = syn::Error::new_spanned(
                &input_fn.sig,
                format!(
                    "#[register_binding] on a function taking a `&mut` big-type or \
                     `#[bind(handle)]` argument cannot also take `{name}` by shared reference. \
                     Take it by `&mut` too, or take it by value."
                ),
            )
            .to_compile_error();
            return TokenStream::from(quote! { #input_fn #err });
    ...
    // The two ways a result ends up in the registry instead of being serialized, and so the exact
    // cases where the caller may want to name the stored item.
    ...
    let output_id_schema = if returns_registry_handle {
        quote! {
            args_schema.push((#OUTPUT_ID_ARG.to_string(), ::process_mining::__rt::serde_json::json!({
                "type": ["string", "null"],
                "title": #OUTPUT_ID_ARG,
    ...
    let stored_id = quote! {
        let id = __output_id
            .unwrap_or_else(|| format!("res_{}", ::process_mining::__rt::uuid::Uuid::new_v4()));
    };

    // A `#[bind(state)]` argument has no schema, so requiring it by name would tell a host to
    // demand an argument it cannot describe and the caller cannot supply.
    let required_arg_names = args_info
        .iter()
        .filter(|a| a.opts.default_value.is_none() && !a.opts.is_state)
        .map(|a| &a.name);
    ...
    let execution_block = if has_any_mut_big_type {
        ...
        let json_extractions: Vec<_> = args_info
            .iter()
            .filter(|a| a.mut_big_type_name.is_none() && !a.is_mut_handle())
            .map(|a| {
                let (name, is_ref, ty_without_ref, opts) = (&a.name, a.change_from_ref, &a.ty_without_ref, &a.opts);
