...
/// through `export_to_path`. A host that offers a download rather than a file dialog -- which is
/// what propel does -- only ever calls the byte path, and a format listed but not handled there
/// fails at the moment a user clicks Export.
#[test]
...
fn picking_the_manifest_opens_the_directory_it_sits_in() {
    ...
    // Through the trait, as a host importing a picked file does -- and it must not be read as
    // the `.json` its name ends with.
...
/// storage a container uses -- the manifest says -- but the filenames still have to round-trip.
#[test]
