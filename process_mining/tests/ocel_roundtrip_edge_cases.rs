...
/// references, `a#b` as a truncated id, and `a{b` as broken JSON -- so the exporter escapes
/// them.
...
fn strict_rejects_an_unknown_o2o_source_without_needing_verbose() {
    ...
    let err = process_mining::core::event_data::object_centric::ocel_csv::
        import_ocel_csv_with_options(csv.as_bytes(), &strict);
...
fn the_bundle_gives_everything_back_exactly() {
    for (label, text) in ORDINARY.iter().chain(RESERVED) {
        let src = log_named(text);
        assert_eq!(facts(&src), facts(&through_bundle(&src, "names")), "{label}");
    }
