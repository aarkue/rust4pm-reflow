...
/// Tries to find a cut in the directly-follows graph of a log, together with the construct that
/// found it.
///
...
pub fn find_cut_with_origin(
    log: &ActivityLog,
    dfg: &ActivityDfg,
    options: &InductiveMinerOptions,
) -> Option<(Cut, Construct)> {
    exclusive_choice_cut(dfg)
        .map(|cut| (cut, Construct::ExclusiveChoiceCut))
        .or_else(|| {
            sequence_cut(dfg)
                .and_then(|cut| match options.strict_sequence {
                    true => strict_sequence_cut(log, cut),
                    false => Some(cut),
                })
                .map(|cut| (cut, Construct::SequenceCut))
        })
        .or_else(|| {
            ...
            concurrent_cut(dfg, minimum_self_distance.as_ref()).map(|cut| {
                if options.use_inclusive_choice {
                    let cut = as_inclusive_choice(log, cut, options.noise_threshold);
                    let construct = if cut.operator() == OperatorType::InclusiveChoice {
                        Construct::InclusiveChoiceCut
                    } else {
                        Construct::ConcurrentCut
                    };
                    (cut, construct)
                } else {
                    (cut, Construct::ConcurrentCut)
                }
            })
        })
        .or_else(|| loop_cut(dfg).map(|cut| (cut, Construct::LoopCut)))
        .or_else(|| {
            options
                .use_interleaved
                .then(|| interleaved_cut(log))
                .flatten()
                .map(|cut| (cut, Construct::InterleavedCut))
        })
}

/// [`find_cut_with_origin`] without the construct.
pub fn find_cut(
    log: &ActivityLog,
    dfg: &ActivityDfg,
    options: &InductiveMinerOptions,
) -> Option<Cut> {
    find_cut_with_origin(log, dfg, options).map(|(cut, _)| cut)
}

/// Cut detection of the Inductive Miner - infrequent (`IMf`), together with the construct that
/// found the cut.
///
...
/// [`find_cut_with_origin`].
pub fn find_cut_filtering_with_origin(
    log: &ActivityLog,
    dfg: &ActivityDfg,
    options: &InductiveMinerOptions,
) -> Option<(Cut, Construct)> {
    if let Some(result) = find_cut_with_origin(log, dfg, options) {
        return Some(result);
    }
    if options.noise_threshold <= 0.0 {
        return None;
    }
    find_cut_with_origin(log, &dfg.filtered(options.noise_threshold), options)
}

/// [`find_cut_filtering_with_origin`] without the construct.
pub fn find_cut_filtering(
    log: &ActivityLog,
    dfg: &ActivityDfg,
    options: &InductiveMinerOptions,
) -> Option<Cut> {
    find_cut_filtering_with_origin(log, dfg, options).map(|(cut, _)| cut)
}
