...
/// Picks the fall through to apply to a log for which no base case and no cut worked, together
/// with the construct that picked it.
///
...
pub fn find_fallthrough_with_origin(
    log: &ActivityLog,
    dfg: &ActivityDfg,
    options: &InductiveMinerOptions,
) -> (Fallthrough, Construct) {
    if !options.use_fallthroughs {
        return (flower_model(log), Construct::Flower);
    }

    activity_once_per_trace(log, options)
        .map(|f| (f, Construct::ActivityOncePerTrace))
        .or_else(|| activity_concurrent(log, options).map(|f| (f, Construct::ActivityConcurrent)))
        .or_else(|| {
            strict_tau_loop(log, dfg, options.use_degenerate_tau_loops)
                .map(|f| (f, Construct::StrictTauLoop))
        })
        .or_else(|| {
            tau_loop(log, dfg, options.use_degenerate_tau_loops).map(|f| (f, Construct::TauLoop))
        })
        .or_else(|| two_activities_concurrent(log).map(|f| (f, Construct::TwoActivitiesConcurrent)))
        .unwrap_or_else(|| (flower_model(log), Construct::Flower))
}

/// [`find_fallthrough_with_origin`] without the construct.
pub fn find_fallthrough(
    log: &ActivityLog,
    dfg: &ActivityDfg,
    options: &InductiveMinerOptions,
) -> Fallthrough {
    find_fallthrough_with_origin(log, dfg, options).0
}
