...
//! connector. Everything here needs [`dbcon`], whose absence of a crates.io release is why the
//! feature is optional and off by default.
//!
//! Every function here takes `connections` -- a map from the `source_id` a `Blueprint`'s nodes
//! name to a `dbcon` connection string (`postgres://...`, `sqlite:...`, or a `.csv` path) -- as a
//! **separate** argument. A blueprint never carries connection details, so the same blueprint can
//! run against staging and then production with no edit.
//!
//! [`extraction_run`] cannot return `(SlimLinkedOCEL, ExtractionReport)` directly: the
//! `#[register_binding]` macro recognises a big type by matching the *whole* return type's name
//! against a fixed list, and a tuple's rendered name never matches. So it takes an
//! already-created, empty `SlimLinkedOCEL` handle (from `locel_new`) as a `&mut` argument and
//! fills it, returning the `ExtractionReport` as an ordinary value.
...
use crate::bindings::extraction_bindings::{merge_into, report_error_message};
...
fn discover_catalog_from_connections(
    ...
    for (source_id, connection_string) in connections {
        merge_into(&mut catalog, discover_catalog(source_id, connection_string)?);
    }
    Ok(catalog)
}
...
/// dynamic type name (one read from a column rather than written in the blueprint) to SQL --
/// see `ExtractionCatalog::with_domain`.
///
...
/// a `&mut` big type and read the registry -- see `StateRef`. The `ExtractionReport` has nowhere
/// to go for the same reason, so a run whose report carries errors fails with them.
///
...
fn extraction_run_items_dbcon(
    ...
    let provider_refs: HashMap<String, &dyn RowProvider> = opened
        ...
        .collect();
    let mut sink = SlimOcelSink::new();
    let report = extract(&blueprint, &catalog, &provider_refs, &mut sink).map_err(|e| e.to_string())?;
...
/// held. Two `source_id`s naming the same item share one guard -- locking the same mutex twice
...
mod tests {
    ...
    /// source `db` -- deliberately built through `Blueprint::from_flat_event_table` so it carries
    /// only a `source_id` string, never a connection string, matching what a real caller sends
    /// over the bindings boundary.
    #[cfg(feature = "ocel-sqlite")]
    ...
    /// Discover, validate and extract a real `SQLite` fixture entirely through the registry --
    /// what a Python or `TypeScript` caller actually does -- and check the returned handle
    /// resolves to a `SlimLinkedOCEL` with the expected contents.
    ///
    /// Doubles as the pinning test for the design property the whole redesign exists for: `bp`,
    /// built by [`flat_blueprint`], is serialized to JSON and back before use, and at no point
    /// carries anything but the source id `"db"` -- the connection string is supplied here, in
    /// `connections`, entirely separately.
    #[cfg(feature = "ocel-sqlite")]
    #[test]
    fn discover_validate_and_extract_a_sqlite_fixture_through_the_registry() {
        ...
        // through JSON to prove that, as a caller relaying it between processes would.
