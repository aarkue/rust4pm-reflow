...
//! without leaking credentials. A binding that needs to reach a real data source takes the
//! connection strings as a **separate** argument, never as part of the blueprint itself.
//!
//! [`extraction_validate`] and [`extraction_compile`] are *pure*: they take a [`Blueprint`] and
//! an already-discovered [`ExtractionCatalog`], open no connection and read no row. The
//! `extraction_*_items` pair reads sources held in the registry as bytes, which needs
//! `ocel-sqlite` but still no connector.
//!
//! # The connected bindings are behind `extraction-dbcon`
//!
...
/// Check `blueprint` against `catalog` for problems decidable from the schema alone -- an
/// unknown source or table, a node graph cycle, a type-rendering rule the blueprint's own
/// `id_rendering` setting cannot satisfy, and the like.
///
...
///
/// `sources` maps a blueprint's `source_id` to the registry id of a [`TabularSource`] -- the bytes
/// of a file someone dropped. Nothing here touches the filesystem, which is what makes it the only
/// extraction path available on `wasm32`.
///
...
fn open_sources<'a>(
    ...
    for (item_id, source_ids) in by_item {
        ...
        let opened = match src.format() {
            ...
            // Bytes, not a path -- so this is the route a browser takes, where a dropped file is
            // all there is. Feature-gated rather than absent, so a build without the connector
            // still says which formats it can manage.
...
/// values a dynamic type name needs -- on wasm, where every source is byte-held, not at all.
///
...
fn extraction_table_preview_items(
    ...
    // `BTreeMap`, so that order is alphabetical -- stable for a table, but not its declared
    // order, and `TablePreview::columns` is what says which is which.
...
///
/// The [`ExtractionReport`] -- drop reasons, per-mapping counts -- is not returned, because a
/// binding returns either a big type or plain data, never both. A run whose report carries errors
...
mod tests {
    ...
    /// source `db` -- deliberately built through
    ...
    /// byte-held, that meant no example values anywhere.
    #[cfg(feature = "ocel-sqlite")]
    #[test]
    fn a_registry_source_reports_its_column_domain_and_a_preview() {
        ...
        // Alphabetical, because the schema keys columns in a `BTreeMap` -- and the rows are
        // aligned to exactly that, which is the point of returning the header alongside them.
        assert_eq!(
            preview.columns,
    ...
    fn a_workbook_registered_as_bytes_discovers_one_table_per_sheet() {
        ...
        let catalog: serde_json::Value =
            serde_json::from_slice(&out).expect("catalog is JSON");
    ...
    /// schema and the required-arguments list. Leaving it in the latter tells every host -- the
    /// CLI checks exactly this list -- to demand an argument it is given no schema for and can
    /// never supply, which makes the binding unreachable.
    #[cfg(feature = "ocel-sqlite")]
    #[test]
