...
pub fn import_ocel_sqlite_from_con_with_options(
    ...
    for (ob_type, ob_type_ocel) in ob_type_map.iter() {
        ...
        let mut s = con.prepare(format!("PRAGMA table_info('{table_name}')").as_str())?;
    ...
    // The maps hand their values out in per-process hash order, and event position is
    // load-bearing downstream: `EventIndex` follows it, and a stable sort by timestamp
    // alone (e.g., when flattening) keeps the pre-sort order for simultaneous events. Left
    // unsorted, the same file yields a different linearization of tied events on every
    // run, and with it different trace variants. Sort to a canonical order instead.
    ocel.objects.sort_unstable_by(|a, b| a.id.cmp(&b.id));
    ocel.events
        .sort_unstable_by(|a, b| a.time.cmp(&b.time).then_with(|| a.id.cmp(&b.id)));
    Ok(ocel)
}
...
mod missing_changed_field_tests {
    ...
    #[test]
    fn default_import_rejects_missing_changed_field_column() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("no-changed-field.sqlite");
        build_fixture(&path);

        let con = Connection::open(&path).unwrap();
        let err = import_ocel_sqlite_from_con(con).unwrap_err();
    ...
    #[test]
    fn opt_in_tolerant_import_reads_contents_correctly() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("no-changed-field.sqlite");
        build_fixture(&path);
