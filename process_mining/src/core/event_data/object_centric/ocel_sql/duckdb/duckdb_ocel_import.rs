...
pub fn import_ocel_duckdb_from_con_with_options(
    ...
    // Same canonicalization as the SQLite importer: hash order here means a different
    // linearization of simultaneous events on every run.
...
mod missing_changed_field_tests {
    ...
    fn default_import_rejects_missing_changed_field_column() {
        ...
        let con = Connection::open(&path).unwrap();
        let err = import_ocel_duckdb_from_con(con).unwrap_err();
