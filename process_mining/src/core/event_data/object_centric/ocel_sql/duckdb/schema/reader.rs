...
///
/// One entry per `(type, name)`. A name reported under two types is widened with
/// [`OCELAttributeType::coalesce`], since no other backend can express the same attribute
/// declared twice.
fn collect_types(
    con: &Connection,
    attr_sql: &str,
    ...
    let mut attrs_by_type: HashMap<String, Vec<OCELTypeAttribute>> = HashMap::new();
    {
        ...
        for row in rows {
            ...
            match entry.iter_mut().find(|a| a.name == name) {
                Some(seen) => {
                    seen.value_type = OCELAttributeType::from_type_str(&seen.value_type)
                        .coalesce(OCELAttributeType::from_type_str(&value_type))
                        .as_type_str()
                        .to_string();
                }
                None => entry.push(OCELTypeAttribute { name, value_type }),
            }
        }
    }
