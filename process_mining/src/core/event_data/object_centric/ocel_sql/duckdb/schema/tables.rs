...
/// Whether `name` clashes with an [`EVENT_BASE_COLS`] entry, directly or as an already-suffixed
/// form of one, which keeps the `_attr` suffixing reversible.
fn collides_with_base_col(name: &str) -> bool {
    EVENT_BASE_COLS.contains(&name.trim_end_matches("_attr"))
}
...
pub(crate) fn event_attr_column(name: &str) -> Cow<'_, str> {
    if collides_with_base_col(name) {
...
pub(crate) fn event_attr_name(column: &str) -> Cow<'_, str> {
    match column.strip_suffix("_attr") {
        Some(name) if collides_with_base_col(name) => Cow::Borrowed(name),
