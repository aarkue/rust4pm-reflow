...
impl Predicate {
    /// Compile the regular expressions in this predicate once, ahead of row evaluation, and
    /// coerce every literal -- in a `Compare` or an `In` alike -- to its column's declared type
    /// where `schema` names one.
    ///
    /// # Literal coercion
    ///
    /// A blueprint's `Literal` is untagged JSON, so an editor's text input for `docstatus = 1`
    /// emits `Literal::Text("1")`, which as-is matches zero rows of an `INTEGER` column. So every
    /// literal position -- a `Compare` against an `Operand::Column`, and each value of an `In`
    /// independently -- is reinterpreted as the column's [`ValueKind`] via [`Value::coerce_to`],
    /// here rather than per row. A literal that does not parse as the target kind is left as
    /// authored and never matches, as is every literal when `schema` is `None`.
    ///
    /// **Part of the model's semantics, not an implementation detail:** a SQL compiler must
    /// reproduce this for every literal position -- cast the literal to the column's SQL type, or
    /// emit an always-false predicate when the cast is impossible -- or the compiled view and this
    /// evaluator will disagree about which rows match.
    ///
    /// # Errors
...
/// Prepare one side of a `Compare`, coercing a `Literal` to `other_side_kind` -- the other
/// side's declared column kind, when there is one -- if it parses cleanly. See
/// [`Predicate::prepare`] for the full rule.
...
/// authored.
///
/// The one coercion rule shared by every literal position -- `Compare`'s operands and `In`'s
/// `values` alike, in this evaluator and in the SQL a compiled view carries. See
/// [`Predicate::prepare`] for the full rule.
...
mod tests {
    ...
    /// `col > 1.0` and `col IN ('NaN')` are all true there -- `Value::compare` has to agree,
    /// or the extractor drops every row a compiled view keeps.
    #[test]
