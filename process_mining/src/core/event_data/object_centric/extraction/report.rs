...
///
/// Deliberately does not include a repeated object id at event grain -- see
/// [`MappingStats::deduplicated`].
#[derive(
...
impl std::fmt::Display for ExtractionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExtractionError::DuplicateObject { mapping, id } => write!(
                f,
                "mapping {}: duplicate object id '{id}'",
                mapping.title()
            ),
