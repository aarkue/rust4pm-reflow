...
//!
//! A [`Blueprint`] declares two things: a flat graph of nodes producing rows, and mappings
//! turning those rows into OCEL events, objects and relations. It deliberately contains no
//! connection details and no schema snapshot -- both are supplied by the caller at execution
//! time, which keeps a blueprint portable and free of secrets.
...
pub use case_centric::{
    event_log_to_ocel, event_log_to_slim_ocel, write_event_log_to_sink, FlatEventTable,
    CASE_OBJECT_TYPE, CASE_QUALIFIER,
};
