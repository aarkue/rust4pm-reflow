...
/// own row iterator -- never collected into a `Vec` first, so a `Source -> Filter` chain over
/// this provider streams.
...
fn deserialize_slice(con: &mut Connection, data: &[u8]) -> Result<(), rusqlite::Error> {
    ...
    // whose header claims WAL mode with SQLITE_CANTOPEN -- `from_slice` would appear to succeed
    ...
    match v {
        ...
        ValueRef::Text(t) => {
            ...
            if declared == Some(DeclaredKind::Timestamp) {
                // Parsed here, once, or left as text for the timestamp cascade to try -- never
...
mod tests {
    ...
    /// database an application left in WAL mode -- the default for a great many of them -- lands
    /// here, and used to load and then fail every scan with "unable to open database file".
    #[test]
