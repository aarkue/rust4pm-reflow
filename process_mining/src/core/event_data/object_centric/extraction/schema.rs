//! Per-node column resolution: what a node's rows look like, and which of its columns are
//! actually read anywhere downstream.
//!
//! Internal to the executor. Two things share the same traversal: the full column shape of every
//! node (needed for [`Predicate::prepare`](super::predicate::Predicate::prepare)'s literal
//! coercion and to disambiguate a `Join`'s `right_<name>` columns), and the *subset* of each
//! node's columns anything downstream reads, so a `Source` node only asks its
...
/// `Filter`'s demand is equal to its input's, not narrower -- see this module's docs.
...
/// [`demanded_columns`]'s entry -- exactly the columns anything downstream reads, with their
