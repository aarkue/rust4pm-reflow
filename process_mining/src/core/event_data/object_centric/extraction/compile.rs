...
//! inlines them as a `WITH` prelude in front of an analysis query (no DDL right needed, so it runs
...
//!   [`Value`](super::value::Value) in each cell. A compiler has only
//!   [`ColumnSchema::declared_kind`](super::catalog::ColumnSchema::declared_kind). The two agree
//!   under a statically-typed engine and not under a dynamically-typed one -- `SQLite` stores a
//!   type per *cell*, so a column declared `TEXT` can hand the extractor a `Value::Integer`.
//! * **Every source table is reachable under its bare name.** A blueprint's `source_id` selects a
//!   connection at extraction time. The emitted SQL names only the table, so a caller compiling a
//!   multi-source blueprint has to attach or alias the sources into one namespace itself.
//! * **A regular expression is emitted verbatim and read by the target engine.** Rust's `regex`
//!   (which the extractor uses), RE2 (`DuckDB`) and POSIX AREs (`PostgreSQL`) do not agree on
//!   every construct, so a `Matches` predicate or a `Regex` split can keep different rows in a
//!   compiled view than in an extraction.
//! * The compiler is a pure function of blueprint plus catalog. It opens no connection and reads
//!   no row, which is why a domain that has to be measured arrives through
//!   [`Catalog::column_domain`](super::catalog::Catalog::column_domain).
...
pub enum EmissionShape {
    ...
    /// `event_object`, `object_object` and the two type maps -- the OCEL 2.0 layout external
    /// tooling reads.
    #[default]
    PerType,
    ...
    /// [`Catalog::column_domain`](super::catalog::Catalog::column_domain) lookup is needed and
...
pub enum RejectReason {
    ...
    /// A node's column shape could not be resolved -- typically an unknown source table.
    ...
    /// A column with no [`Value::canonical_string`](super::value::Value::canonical_string) --
    /// `Float` or `Timestamp` -- is used at an identity position, where it makes the expression
    /// `None` for every row.
    ...
    /// does not reproduce -- Rust writes `1` where `DuckDB` writes `1.0`, and keeps a
    /// timestamp's original offset -- feeds a position that reads it as text.
...
impl CompiledOcel {
    ...
    /// whose dependencies could not be ordered -- reachable only from a blueprint that skipped
    /// [`validate`](super::validate) -- is dropped and reported in [`Self::errors`] as
    ...
    /// existing -- no DDL right, so it runs against a read-only database.
    ///
...
/// are load-bearing here -- the version check (which lives only there), duplicate node ids (which
/// would make the emitter read a node's op and its columns from two different declarations) and
/// node cycles or unknown tables (which would otherwise degrade to a silently empty log).
#[must_use]
pub fn compile(
    blueprint: &Blueprint,
    catalog: &dyn Catalog,
...
/// else `String` -- `resolve_attribute_type` in `mapping_exec`.
...
/// What an identity relation -- `PerType`'s `object` and `event`, `Consolidated`'s `objects` --
/// reads off one branch, so one body builder serves both branch types.
trait EntityBranch {
    fn id_sql(&self) -> &str;
...
impl MappingCtx<'_> {
    ...
    /// rejected -- and, under [`MissingEndpointPolicy::Create`], the object branch that creates it
    ...
    fn compile_o2o(
        ...
        // guards -- but not its type guard, since the source is created before the target's type
        // is ever rendered. The created branch reads the target's `FROM`, the only one those
        // guards can be evaluated against.
...
/// -- `object_map_type` is `object_` plus a type literally named `map_type`.
fn check_reserved(kind: &'static str, names: &[String]) -> Result<(), RejectReason> {
    for name in names {
        ...
        if RESERVED_RELATIONS.contains(&view.as_str())
            || RESERVED_RELATIONS.contains(&name.as_str())
        {
...
fn constant_names(type_sql: &str) -> Vec<String> {
    ...
    match trimmed {
        Some(inner) if !inner.contains('\'') => vec![inner.to_string()],
        _ => Vec::new(),
    }
}
...
/// Reorder `pending` so every view comes after each view named in its own dependency set --
/// Kahn's algorithm, in the same style as the cycle sweep `validate.rs` runs over the node
/// graph, but building an order instead of only detecting a cycle.
///
...
/// Render a `declared`-typed SQL expression as the text `to_sql_value` would produce, so
/// `from_sql_value` -- what
/// [`DuckDbLinkedOCEL`](crate::core::event_data::object_centric::ocel_sql::DuckDbLinkedOCEL)'s
