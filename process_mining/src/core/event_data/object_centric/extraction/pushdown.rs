...
//!
//! The SQL comes from [`Emitter`], the compiler's own emitter, rather than a second generator:
//! drift between two of them would show up as a memory optimisation changing results. Reusing it
//! costs one wrapper -- [`Emitter::node_sql`] projects a node's **full** column set while the
//! executor's rows carry only the demanded subset (see [`super::schema`]), so the query is wrapped
//! in a projection naming exactly the executor's columns in its order.
//!
//! # The catalog precondition
//!
//! The emitter decides everything from *declared* column types, so its SQL matches the row-level
//! path only when the source's runtime values have the kinds the catalog declares. The provider
...
/// is safe -- the caller executes the node row by row -- but the fall-back is `hash_join`, whose
/// memory grows with the data, so the reason is carried out rather than discarded.
