...
pub fn validate(blueprint: &Blueprint, catalog: &dyn Catalog) -> Vec<ValidationError> {
    ...
    for node in &blueprint.nodes {
        ...
        if let NodeOp::Filter { input, condition } = &node.op {
            errors.extend(check_literal_kinds(
                &format!("filter '{}'", node.id),
                condition,
                schemas.get(input.as_str()),
            ));
        }
    }
    ...
    for (path, mapping) in &desugared {
        ...
        if let Some(when) = &mapping.when {
            ...
            errors.extend(check_literal_kinds(
                &format!("mapping {name}"),
                when,
                schemas.get(mapping.node.as_str()),
            ));
        }
    }
...
/// Report every node genuinely on a cycle -- not merely downstream of one, which needs no
/// separate error since fixing the cycle fixes it too.
...
/// Whether following dependency edges from `start` can lead back to `start` -- true exactly for
/// a node genuinely on a cycle, as opposed to one merely reading from a node that is.
...
/// Every [`ValueExpression`] position a target names -- an id, a type, a qualifier or a timestamp
/// -- without recursing into `Coalesce`'s parts. Callers that need to look inside one call the
/// expression's own methods on what they're handed.
...
/// Every regular expression in the blueprint that must compile to run it: a `Matches` pattern
/// anywhere -- a node's `Filter` condition or a mapping's `when` -- and a `Regex` split on any
/// `ObjectEndpoint`. One traversal driven from both node ops and mappings, so a new regex-bearing
/// position only has to be added here to be checked everywhere, rather than risking a second
...
mod tests {
    ...
    fn a_literal_that_cannot_be_read_as_its_column_s_type_is_reported() {
        ...
        // whatever the data holds -- and nothing at run time can tell that apart from an empty
        // result.
