...
/// populate it. Every test gets its own directory -- a shared fixture path corrupts under
...
/// schema -- and every mapping reading the filter then indexed the wrong slot, silently.
///
...
/// input -- so it was never fetched from the left, and materialised as `Null` -- while the
/// runtime rule resolved it from the left.
#[test]
...
/// *projected* left schema, which by then did not, and `right_foo` materialised as `Null` -- the
/// row dropped for an unusable id. Both rules now ask both sides' **full** schemas.
#[test]
...
/// which is `None` for floats, so the join silently produced zero rows -- while SQL joins them.
#[test]
...
///
/// A `Target::Event` with `id: None` cannot emit its references in the relations pass -- a
/// run-minted UUID cannot be re-derived there -- so they are emitted in the events pass, which used
/// to be the same pass as objects -- where
/// a `Target::Object` mapping written *after* the event mapping had not run yet. With
...
fn c3_an_inline_object_reference_resolves_against_every_object_whatever_the_order() {
    ...
    let build = || {
        ...
        // whatever has not been emitted yet. `Drop` is what exposes it -- and is a setting an
        // author may legitimately choose for the same blueprint.
...
/// references were processed, so those relations vanished with no `DropReason` -- contradicting
/// They belong to the row, not to the event insertion, and a compiled relation view emits
/// one per row regardless of how many distinct events the id column names.
#[test]
...
/// I-c: `deduplicated` used to count every relation endpoint that resolved, so the *normal*
/// successful case -- an `E2O` mapping over n rows, every endpoint present, every id distinct --
/// reported n deduplications while nothing had been deduplicated.
#[test]
...
/// writing the second's attributes onto it failed the moment the first type had not declared that
/// attribute name, aborting the *entire* extraction with `?`.
#[test]
...
/// identical blueprint through a deferring sink -- this is the shape whose two sinks disagreed.
...
/// event -- there is no timestamp to give it -- so it degrades to `Drop` there while still
/// creating objects.
#[test]
...
/// deferral branches -- the half of the sink-agreement invariant a fan-out run cannot reach at all (see
...
fn assert_both_sinks_agree_on_separate_runs(
    ...
    // The log snapshot is content, not counters -- it cannot catch two sinks agreeing on what
...
/// author-given is comparable this way -- a minted event id differs between runs by design --
...
/// source id, and evaluated that gate against `objects` -- from inside the very `INSERT INTO
/// objects` that creates the source -- so the source was never there, the target ask was ruled
...
/// `duckdb_sink`'s own test module, where it drove `DuckDbSink` alone -- pinning the sink under
/// test rather than the reference semantics -- and gave its "unresolvable" source a type that
/// `Create` can perfectly well create, so the eager path *did* reach the target ask and the
/// asserted type was simply wrong.
///
...
/// target. The deferring sink still staged that target ask, and it names `shared` -- an id a real
/// `E2O` also names, later and under a different type. Ungated, the earlier, unreachable ask wins
...
//
// `ExtractionReport::rows_materialized` is the direct, deterministic witness for the sink-agreement invariant
// -- see graph.rs's `GraphExecutor::rows_materialized`. An earlier version of this test asserted
// on process-wide resident memory growth instead; that reading is not attributable to this
// code alone when other tests are allocating concurrently in the same process (`cargo test
// --lib` runs tests on a thread pool by default), so it failed intermittently depending on
// scheduling. Asserting on the executor's own materialisation count is immune to that: it is
// computed purely from what this one `extract` call did.
// ---------------------------------------------------------------------------------------------
...
/// makes -- `run_e2o` gives up on row 1 before it ever looks at the object endpoint, because the
/// event does not resolve.
#[cfg(feature = "ocel-duckdb")]
...
/// same fixture used to trigger -- the `rels` mapping's own `deduplicated` count differing between
/// the sinks, because `resolve_object_endpoint` recorded the ghost ask at row-processing time and
...
/// `resolve_object` time, so it always takes the "maybe I already own this id" branch -- and used
/// to treat the `add_object` rejection that follows as "the id already exists, append to it",
...
/// persisted declaration -- a pre-existing, separate limitation this fixture happens to trigger,
/// not the content-merge bug this test is pinning.
#[cfg(feature = "ocel-duckdb")]
#[test]
...
/// Duplicate node ids read differently in two places -- `Blueprint::node` takes the first match,
/// `full_node_schemas` lets the last win -- so `node_sql_inner` took the op from one node and the
...
/// statically-named type up front precisely so "the declared type set is a function of the
/// blueprint alone, not of which rows happen to match". `case_2`'s `CreditNote` is exactly that
/// type, and until now the fixture only ever ran through `SlimOcelSink`.
///
/// **Known divergence, not closable from this module.** The consolidated `DuckDB` layout has
...
/// `ocel_sql::duckdb::schema::tables` and a declared-types union in that module's reader -- a
...
/// Two event types, `A` and `B`, both declaring an attribute `n` -- `A` as an integer, `B` as a
