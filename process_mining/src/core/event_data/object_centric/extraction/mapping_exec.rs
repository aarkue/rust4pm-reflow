...
pub(crate) struct RunCtx<'a> {
    ...
    /// Event type names this mapping has already declared -- local to one mapping's run, reset
    /// per mapping. See [`ensure_event_declared`].
    ...
    /// with -- run-level, not per mapping, which is the point: see [`reconcile_attr_types`].
...
/// Prepare every [`ObjectEndpoint`] a target names, in the same order
/// [`target_object_endpoints`] walks them -- the order this module's `run_*` functions index
/// into by position.
...
///
/// **Unless the event's `id` is `None`**: the id is a freshly minted `UUID`, so the relations pass
/// could not re-derive which event a row produced, and the references go in the events pass
/// instead -- sound because the objects pass has already run.
...
/// What a [`Target::Object`] row does when the object it names is already there, however the sink
/// said so -- an eager `Exists`, or a deferring sink's `add_object` rejection.
///
...
/// Resolve an object endpoint to a handle, applying `on_missing_endpoint` -- the single place
/// every object-relating position (inline references, `E2O`'s object, `O2O`'s source/target)
/// goes through, so the policy is honoured identically at each.
///
...
///
/// `Create` cannot synthesise a missing event -- there is no timestamp to give it -- so it
...
/// Overwrite `buf` with this row's values for `attributes`, reusing the buffer's allocations --
/// the `Vec` and each name `String` -- since one mapping's attribute list is the same every row.
fn fill_event_attrs(
...
/// tell whoever reads the report opposite things -- fix your format, versus these rows have no
/// date.
