...
//!
//! | node | held while it runs |
//! | --- | --- |
//! | `Source`, `Filter` | one row |
//! | `Union` | one row |
//! | `Join`, pushed down to its source | one row |
//! | `Join`, executed here | its **right** input |
//!
//! A `Join` is the only node that must hold something: matching rows means one side has to be
//! available in full while the other is scanned past it. It holds the **right** input and streams
//! the left. When both of its inputs read one source, [`super::pushdown`] asks that source to do
//! the join instead and neither side is held.
//!
//! Nothing is cached between nodes: a node read by two consumers is executed twice. See
//! [`GraphExecutor::stream`].
//!
//! Which side a join builds is the author's call rather than a heuristic's: **put the smaller
//! table on the right.** Getting it backwards costs memory, never correctness, while counting both
//! inputs first would mean a full extra scan of both tables on every run.
//!
//! A join resolves both key column sets and every output column before reading a row, and any that
//! does not resolve is an error. Skipping a key column would shorten the key into a partial cross
//! product, and null-filling an output column would report a run that dropped a mapping's whole
//! input as a success.
//!
//! A [`Union`](super::blueprint::NodeOp::Union) concatenates its inputs' rows, keeping duplicates.
//! Its output columns are the union of its inputs' column *names*, and an input lacking one
//! contributes `Null` for it. Both halves are semantics the compiler must reproduce: `UNION ALL`
//! rather than `UNION`, and an explicit null fill rather than positional alignment.
...
struct BuildSide {
    ...
    /// many-to-many join, and every one of them is paired with each matching left row.
...
impl<'a> GraphExecutor<'a> {
    ...
    /// Ask `node_id`'s source to execute the whole node -- join, filters and all -- and stream
...
mod tests {
    ...
    /// database -- and, unlike the end-to-end tests, without `validate` first rejecting the
    /// deliberately malformed graphs below.
    #[derive(Debug)]
    ...
    /// Silently dropping that pair from the key -- what `filter_map` used to do -- shortens the
    /// key, so rows agreeing on only the *remaining* columns get paired: a partial cross product
