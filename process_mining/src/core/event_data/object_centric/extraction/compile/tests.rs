...
//! refused. That the SQL agrees with the extractor is checked in `compile::differential` alone.
#![cfg(test)]
...
/// actually exist" check -- so the row `assemble` adds to `object` semi-joins `event`, and
/// `object` must be emitted after `event` even though the fixed old order put it first.
...
fn a_cross_kind_join_key_compiles_to_a_join_that_matches_nothing() {
    ...
    // an Integer 1 -- while DuckDB would implicit-cast and join them.
    let catalog = ExtractionCatalog::new()
...
fn postgres_uses_its_own_function_names() {
    let pg = SqlDialect::Postgres;
    // `trim(str, chars)` is DuckDB-only; PostgreSQL spells the two-argument form `btrim`.
    assert!(pg.unicode_trim("x").starts_with("btrim(x, '"));
