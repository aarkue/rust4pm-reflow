...
/// The complete Unicode `White_Space` set (25 codepoints), matching exactly what Rust's
/// `char::is_whitespace` -- and therefore [`str::trim`] -- strips.
///
/// `DuckDB`'s one-argument `trim` only strips ASCII space, so a split with `trim: true` must pass
/// this set explicitly to be byte-identical to the extractor's
/// [`PreparedSplit::split`](crate::core::event_data::object_centric::extraction::expr::SplitSpec).
pub(crate) const UNICODE_WHITE_SPACE: &str =
    "\u{0009}\u{000A}\u{000B}\u{000C}\u{000D}\u{0020}\u{0085}\u{00A0}\
     \u{1680}\u{2000}\u{2001}\u{2002}\u{2003}\u{2004}\u{2005}\u{2006}\u{2007}\u{2008}\u{2009}\
...
impl SqlDialect {
    ...
    /// A parenthesized subquery in `FROM` position, always aliased -- `PostgreSQL` and `MySQL`
    /// both reject an unaliased derived table, and `DuckDB` merely tolerates it.
    ...
    /// Strip the full Unicode `White_Space` set, matching Rust's [`str::trim`].
    pub(crate) fn unicode_trim(self, expr: &str) -> String {
        match self {
            SqlDialect::DuckDb => {
                format!("trim({expr}, {})", self.string_literal(UNICODE_WHITE_SPACE))
            }
            // `btrim(string, characters)`, the two-argument form; `trim(expr, chars)` is a DuckDB
            // spelling PostgreSQL does not accept.
            SqlDialect::Postgres => {
                format!(
                    "btrim({expr}, {})",
                    self.string_literal(UNICODE_WHITE_SPACE)
                )
            }
        }
    }
    ...
    pub(crate) fn regex_split_to_rows(self, expr: &str, pattern: &str, groups: usize) -> String {
        match self {
            ...
            // groups, the capture groups otherwise -- the same shape DuckDB's branch produces.
