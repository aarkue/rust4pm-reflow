...
//!
//! Everything here mirrors a specific piece of the extractor, named in the implementing
//! function's doc comment. Where the two could disagree the function returns a [`RejectReason`]
//! rather than guessing.
//!
//! # The catalog precondition
//!
//! The extractor decides literal coercion, identity rendering and join-key matching from the
//! **runtime** [`Value`] in a cell, while a compiler has only [`ColumnSchema::declared_kind`].
//! Every kind-dependent rule below therefore assumes the catalog describes the kinds a source's
//! values actually have -- true of a statically typed engine, not of `SQLite`, which stores a type
//! per cell.
...
impl<'a, 'b> Emitter<'a, 'b> {
    ...
    /// An inner join whose output columns are routed by [`join_column_source`] -- the same rule
    /// the extractor's own `GraphExecutor` uses, so `right_<name>` cannot mean two things.
    ///
...
/// Whether a timestamp column carries no offset of its own -- `TIMESTAMP`, `DATE`, `DATETIME` --
/// so it needs the explicit UTC anchor [`SqlDialect::timestamp_column`] applies.
...
/// A column operand's declared kind, or `None` for a literal -- exactly `column_kind` in
/// `predicate.rs`, which is what decides whether the *other* side's literal is coerced.
fn operand_kind(
    operand: &Operand,
    schema: &TableSchema,
