...
//!
//! `compile::tests` checks the *shape* of emitted SQL. Only these tests check that the SQL
//! *means* the same thing the extractor does.
//!
//! Both sides are reduced to the same [`LogSets`] and compared field by field. Values are
//! rendered through one kind-tagged function used on both sides, so a `Text` `"1"` cannot pass for
//! an `Integer` `1`. Sets, not multisets: every relation view is `DISTINCT`, since the extractor's
//! identity rules make a repeated `(event, object, qualifier)` indistinguishable from a single one.
//!
//! The extractor runs against the fixture through `DuckDbRowProvider`, a test-only
//! [`RowProvider`] over the very connection the compiled SQL is then executed on, so a divergence
//! can only come from the compiler. That also satisfies the compiler's catalog precondition by
//! construction: `DuckDB` types its columns, not its cells.
//!
//! One known extractor limitation is pinned down rather than avoided: under `IdRendering::Raw`
//! two entities of different types can claim one id, where the extractor reports
//! `IdTypeCollision` and drops the loser while the compiled views keep both. `assert_agrees`
//! asserts the report contains no `IdTypeCollision` before comparing, so `Raw` blueprints stay
//! testable and a fixture that grows a collision fails with a named reason.
...
/// function
/// [`DuckDbLinkedOCEL`](crate::core::event_data::object_centric::ocel_sql::DuckDbLinkedOCEL) uses.
...
/// `Null`" and "belongs to a different event type" -- the schema's own limit, which every reader
/// of it shares. Object attributes have no such gap, since `object_attribute_changes` is EAV.
...
fn case_3_an_ordered_group_keeps_the_rows_whose_guard_column_is_null() {
    ...
    // that row -- the reason this fixture has a NULL in the guard column at all.
    let fx = Fixture::new(DOCS);
    let bp = blueprint(
        IdRendering::TypePrefixed,
        vec![source("docs", "docs")],
...
fn case_7_a_text_literal_against_an_integer_column_selects_the_same_rows() {
    ...
    // outright -- pinned by the assertion at the end of this test.
    let fx = Fixture::new(ORDERS);
    let bp = blueprint(
        IdRendering::TypePrefixed,
        vec![source("orders", "orders")],
...
fn case_9_the_same_blueprint_agrees_under_both_emission_shapes() {
    ...
    // And the relation *names* genuinely differ -- this is not two views of the same SQL.
...
fn case_11_all_four_consolidated_emission_paths_agree() {
    ...
    // rights -- a relation referenced more than once is computed once instead of re-inlined.
...
/// Whether `compare` -- one of the real comparison entry points, handed a deliberately tampered
/// log -- actually rejects it.
///
...
fn every_probe_kind_is_shown_to_fire() {
    ...
    // The extractor really does disagree here -- the probes are announcing a genuine divergence,
    ...
    assert_ne!(
        ...
        "the extractor drops the losing object and the views keep it -- that is what \
...
fn a_naive_date_compared_in_a_predicate_is_read_as_utc_not_in_the_session_zone() {
    ...
    assert_eq!(
        ...
        "row 1's date is UTC midnight, which is before 02:00Z. Read in the session zone it \
         would be 05:00Z and the view would drop the row -- the boundary is deliberately \
