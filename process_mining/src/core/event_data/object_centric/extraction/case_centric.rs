...
/// seen under that name -- the schema a sink has to be given before any entity of the type.
///
...
pub struct EventLogWriteReport {
    ...
    /// How many of those carried no usable `time:timestamp` -- absent, or holding something other
    /// than a date -- and were written at the Unix epoch.
    ///
...
/// none.
///
/// The log is read twice -- once for each type's attribute schema, which a sink must be told
/// before it is given anything of that type, and once to write. Only the schema is held between.
///
...
mod tests {
    ...
    fn case_attributes_land_on_the_object_and_event_attributes_on_the_event() {
        ...
        // is declared from every event of that type at once -- but not one no event carried.
