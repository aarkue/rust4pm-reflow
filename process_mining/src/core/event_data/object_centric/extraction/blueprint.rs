...
pub enum BlueprintParseError {
    ...
    /// `Blueprint` -- including an unknown variant within a supported version, which is a
    /// genuine parse failure, not something this method's version check is meant to catch.
...
mod tests {
    ...
    fn from_json_still_reports_an_unknown_variant_within_a_supported_version() {
        ...
        // does not implement, and serde's message is the right diagnostic for that.
        let json = r#"{
          "version": 1,
          "nodes": [
            { "id": "a", "op": { "type": "a-future-node-op-this-build-does-not-know" } }
