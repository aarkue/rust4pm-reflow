...
impl ColumnSchema {
    ...
    /// [`Predicate::prepare`](super::predicate::Predicate::prepare).
    ///
    ...
    ///
    /// Per word rather than by substring: `interval` and `point` contain `int`, `daterange`
    /// contains `date`, and a wrong kind here is silent -- it coerces a literal and drives the
    /// compiler's `CAST`.
    #[must_use]
