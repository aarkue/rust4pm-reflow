//! Streaming [`ExtractionSink`] writing straight to a `DuckDB` file, reusing the consolidated
//! schema and value conversions from [`ocel_sql::duckdb::schema`](crate::core::event_data::object_centric::ocel_sql::duckdb::schema).
//!
//! # Endpoint resolution is deferred
//!
//! [`SlimOcelSink`](super::slim_sink::SlimOcelSink) resolves a relation endpoint through an
//! in-memory id index. A sink that streams to disk has no such structure, and building one would
//! reintroduce exactly the memory ceiling this sink exists to remove. So it answers
//! [`Resolution::Deferred`] to every [`resolve_event`](ExtractionSink::resolve_event) /
//! [`resolve_object`](ExtractionSink::resolve_object) call, lets the relation be written
//! unresolved, and settles all of them at [`finalize`](ExtractionSink::finalize) with a handful of
//! set-based joins. The schema makes that safe: `e2o`/`o2o` carry no foreign key onto
//! `events`/`objects`, so an unresolved relation row is insertable and later removable.
//!
//! Under [`MissingEndpointPolicy::Create`], and only then, each object endpoint it was asked
//! about is also recorded in a staging table, since that is the one policy whose `finalize` has to
//! know what type an id was named under.
//!
//! The cost is one rejected insert per repeated object id, where an id index would let this sink
//! skip the attempt.
//!
//! **`on_missing_endpoint` is honoured identically to the eager sink**, but at finalize rather
//! than at the call site. `Drop` and `Error` delete the relations whose endpoints did not resolve.
//! `Create` first synthesises the missing objects from the type the endpoint declared, then
//! deletes only what is still unresolved. What differs is where the number is reported: an eager
//! sink's losses land in each mapping's [`DropReason::UnresolvedEndpoint`](super::report::DropReason)
//! tally, this one's in [`FinalizeReport::unresolved_endpoints`].
//!
//! Duplicate ids are *not* deferred: `events.id` and `objects.id` are `PRIMARY KEY`, so `DuckDB`
//! itself rejects a repeat and this sink reports it as
//! [`SinkError::DuplicateEvent`]/[`SinkError::DuplicateObject`] exactly when the in-memory sink
//! would, which keeps `deduplicated` identical across both.
//!
//! # Two known divergences from the eager sink
//!
//! The wide `events` table's attribute columns are typed once, from whatever
//! [`declare_event_type`](ExtractionSink::declare_event_type) calls have arrived by the first
//! [`add_event`](ExtractionSink::add_event). A name declared after that is added on demand, typed
//! from the accumulated declaration. The two sinks can still disagree if a later declaration gives
//! an already-declared attribute name a genuinely different type *and* arrives after the first
//! `add_event` froze the column.
//!
//! [`resolve_object`](ExtractionSink::resolve_object) cannot check an endpoint's declared type
//! against the type the object actually has, since it never looks the object up. For a
//! `Target::Object` row [`add_object`](ExtractionSink::add_object) recovers this one call later
//! and reports [`SinkError::IdTypeCollision`]. A *relation* endpoint has no equivalent add-time
//! check, so under [`IdRendering::Raw`](super::blueprint::IdRendering::Raw) an id shared by two
//! object types resolves here as whichever type already holds it, where the eager sink reports a
//! collision. [`IdRendering::TypePrefixed`] removes the possibility outright.
...
///   makes.
/// - `run_o2o` never reaches a row's targets when its *source* cannot be resolved or created --
///   reachable under [`MissingEndpointPolicy::Create`] whenever the source endpoint's
///   `object_type` expression yields nothing on that row, so there is nothing to create it as.
///
/// Left in [`T_DEFERRED`]'s `arg_min` unfiltered, such an ask could win the type for an id that
/// *is* reachable through a different, real row -- picking whichever ask merely arrived first,
/// not the one the eager path would have made. [`DuckDbSink::resolve_deferred`] excludes an ask
/// whose gate row names an entity that neither exists nor is about to be created. See the two
/// strata there for why an `'object'` gate cannot simply test `objects`.
///
/// What this does not close: the gate's predicate is satisfied by any id present in `objects`,
/// whatever type that row carries, so it lets through the target ask of an `O2O` whose source id
/// is taken by another type -- the same blind spot the module docs describe for relation
/// endpoints. `IdRendering::TypePrefixed` removes it outright.
///
...
/// **first** value written at an instant and silently discards a repeat -- the contract on
/// [`add_object_attribute`](ExtractionSink::add_object_attribute).
///
...
/// given -- it has no index to hand back instead.
///
...
/// surface -- unlike `events`/`objects`, whose primary key is how this sink reports a repeated id
/// at the call site. Amortising the appender over a chunk is what an appender is for.
...
impl DuckDbSink {
    ...
    pub fn new(path: impl AsRef<Path>) -> Result<Self, SinkError> {
        ...
        // reports a repeated id, and the [`I_OBJECT_ATTR_ONCE`] rejection that is how first-wins
        // on `(id, name, time)` is enforced. Under one transaction the first static object
        // attribute written twice would fail every statement after it.
        Ok(Self {
            con,
    ...
    /// other policy every staged row is written and then dropped unread -- two appender
    /// create-and-flush cycles per relation endpoint, for nothing.
    ...
    fn resolve_deferred(&mut self) -> Result<FinalizeReport, duckdb::Error> {
        ...
        if self.on_missing_endpoint == MissingEndpointPolicy::Create {
            // One object per still-missing id, typed from the *first* reachable ask -- matching
            // the eager path, where the first ask creates the object and a later ask under
            // another type is an `IdTypeCollision` rather than a retype.
            //
            // "Reachable" is load-bearing, not merely "staged here". This sink was asked about
            // every endpoint a row named, including ones the eager path never touches at all --
            // `run_e2o` gives up before looking at its object endpoint when the *event* endpoint
            // does not resolve, and `run_o2o` never reaches a row's targets when its source cannot
            // be resolved or created -- while this sink, deferring everything, cannot tell in
            // advance and stages the ask anyway. An unreachable ask must not win `arg_min` merely
            // for having a smaller `seq`, so `T_ENDPOINT_GATE` rules those asks out before a type
            // is picked, and the final `WHERE` still confines *creation* to ids a surviving
            // relation actually names, so an id with no reachable ask at all is never synthesised
            // for relations deleted moments later.
            //
            ...
            //
            // - `'event'` tests `events`, which this statement does not write. Final already.
            // - `'object'` tests the *post-creation* object set, not `objects`: under `Create` a
            //   relation-only source is created by this `INSERT`, so testing `objects` -- which is
            //   read pre-creation -- rules out the target ask of every synthesised source, and the
            //   target is then never created and the relation deleted for want of it.
            //
            // The post-creation set is `available` below (`objects UNION creatable`), but the gate
            // cannot reference it: `available` is derived from `creatable`, which is derived from
            // `reachable`, which is what the gate filters -- a cycle SQL will not evaluate. It is
            // stratified instead. An `O2O`'s *source* ask is never gated (see `T_ENDPOINT_GATE`),
            // so a source's own creatability is settled by the event stratum alone: `gate_available`
            // -- `objects` plus every id with an event-reachable typed ask -- answers the object
            // gate without needing `creatable`, and the strata terminate.
            //
            // It over-approximates in exactly one place, on purpose: an id whose only typed ask
            // belongs to some *other* relation counts as available, even though that ask might
            // itself be gated out. The eager path is order-dependent on precisely that shape (the
            // source does exist, if the relation that creates it happens to run first), so no
            // set-based rule can match it row-for-row anyway.
            //
            // Both directions of that trade can cost a whole object and its relation, not merely a
            // type guess. Erring towards available: an O2O source whose type expression is NULL on
            // the row, whose id is also typed by a later E2O whose event resolves, lands in
            // `gate_available` through that E2O ask -- so the target is created and the o2o kept,
            // where the eager path (o2o mapping first) produced neither. Erring the other way
            // loses the relation whenever the source is itself relation-only, which is the
            // regression this stratification exists to fix. The trade is unwinnable; this side is
            // chosen because it is the one that keeps `Create` working for the common shape.
    ...
    /// not happen through [`extract`](super::extract::extract) -- every name `add_event` is ever
    ...
    /// object's actual one (module docs) is recovered from -- one query, only on an actual
    /// conflict, not a per-id structure held for the run.
    ...
    /// [`SlimOcelSink`](super::slim_sink::SlimOcelSink) still holds `Null`. That is a documented,
    /// accepted caveat of `write_ocel_to_duckdb`'s own round trip, but it is a *divergence*
    /// between the two extraction sinks, which must agree. `"null"` is
    ...
    /// an `E2O`, the source object id for an `O2O`. Consumes the pending seq, so a second
    /// relation written without an intervening ask gates nothing.
    ///
    /// A failure to record the gate row is swallowed, but it is *not* harmless: an ungated ask can
    /// win `arg_min` for an id a real row also names, so `Create` synthesises that object under a
    /// type the eager path would never have used, and both sinks then hold the same id under
    /// different types. (The relation itself survives either way -- only the *type* moves. The
    /// opposite mistake, an over-eager gate, is what costs a relation: see `resolve_deferred`.)
    /// It is swallowed anyway because failing the whole extraction over an appender error here
    /// would be worse, and because nothing this sink can do at this point would repair the ask.
...
impl ExtractionSink for DuckDbSink {
    fn declare_event_type(
        ...
        // `collect_types` keeps the *first* row per pair -- from a `SELECT` with no `ORDER BY`,
        // so "first" was not even stable. Deleting the pair before appending collapses both
        // rules onto the same answer.
        let mut stmt = self
            .con
            .prepare(&format!(
    ...
    /// type whose mapping matched nothing -- which [`extract`](super::extract::extract) declares
    /// up front on purpose. Rebuilding either from the observed change rows loses both, and
    ...
    fn add_o2o(
        ...
        // on `ExtractionSink`), so the same gate that saves `E2O` covers `O2O` too -- keyed on
        // the source, which is what the eager path gives up on before it reaches any target.
...
mod tests {
    ...
    /// finalize under `Drop`, and counted -- the same loss an eager sink reports per mapping as
    ...
    /// `on_missing_endpoint: Create` is honoured at finalize, from the type the endpoint declared
    /// -- which is why `resolve_object` is given it.
    #[test]
    ...
    fn a_column_declared_after_the_freeze_still_gets_its_declared_type() {
        ...
        // "m" -- exercises the on-demand ALTER path, not "n" itself (widening an *existing*
    ...
    /// `SlimOcelSink` keeps the **last** declaration for a `(type, attribute)` --
    ...
    // The `O2O` half -- that a target ask staged for a row whose source never resolves must not
    // win `arg_min` -- is covered by `case_11_an_o2o_target_ask_for_an_unresolvable_source_does_not_win_the_type`
    // in this module's `tests` sibling, through `assert_both_sinks_agree_on_separate_runs`. It
    // used to live here, driving this sink alone: that pinned this sink's own output rather than
    // the eager semantics it must match, and its "unresolvable" source declared a type `Create`
    // can create, so the eager path did reach the target ask and the asserted type was wrong.
    // A source is genuinely unresolvable under `Create` when its type expression yields NULL,
    // which is what the rewritten test uses.
