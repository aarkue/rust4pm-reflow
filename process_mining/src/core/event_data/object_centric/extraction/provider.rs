...
pub trait RowProvider: Debug {
    ...
    /// The SQL dialect this provider can execute through [`Self::scan_query`], or `None` -- the
    /// default -- when it cannot run SQL at all.
    ///
    ...
    /// values really have the declared kinds. A dynamically typed engine -- `SQLite` stores a type
    /// per cell -- must therefore keep the default `None`, or a pushed-down join could return
    /// different rows than the row-level path.
    fn query_dialect(&self) -> Option<SqlDialect> {
        None
    }

    /// Run `sql` -- a single `SELECT` in this provider's [`Self::query_dialect`], whose result
    /// columns are exactly `columns` in exactly that order -- and call `f` once per result row,
    /// under the same push-based, [`ControlFlow`]-honouring contract as [`Self::scan`].
    ///
...
pub enum ProviderError {
    ...
    /// query it was handed is one it cannot run. Never fatal on its own -- the caller falls back
    /// to executing the node row by row.
    QueryUnsupported,
...
///
/// Unlike [`preview_rows`] this must be complete -- the compiler lowers a dynamic type name from
/// the domain, and a partial answer would silently drop rows -- so it reads the whole column. A
/// provider whose backend has `SELECT DISTINCT` should prefer that. See
