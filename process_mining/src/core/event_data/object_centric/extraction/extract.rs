...
///
/// "Whichever runs first" is **not** mapping order. Within a phase, mappings are grouped by the
/// node they read so each node is scanned once, and the groups run in first-seen node order across
/// the whole desugared list -- so the real order is `(phase, node position, mapping index)`. Adding
/// an unrelated mapping on a different node can move that node earlier and flip which of two
...
/// Declare every statically-named (`Constant`) event/object type one mapping's target names --
/// its own type if it has one, and every relation endpoint's type -- so `Create`-policy
/// synthesis never races an undeclared type, and a zero-match mapping still declares its type.
