...
//!
//! # Floats follow SQL's total order, not IEEE's
//!
//! `DuckDB` and `PostgreSQL` both place `NaN` above every other value and make `NaN = NaN` and
//! `-0.0 = 0.0` true, where IEEE's `partial_cmp` answers `None`. Answering `None` would make every
//! operator false -- `Ne` included, where SQL says true -- and would give `-0.0` and `0.0` two
//! different join keys, losing rows a compiled view keeps. [`Value::compare`] and
//! [`Value::join_key_part`] therefore both follow the engines. Reachable from a plain-JSON
//! blueprint, since a `Literal::Text` `"NaN"` against a `DOUBLE` column coerces to
//! `Value::Float(f64::NAN)`.
//!
//! # A join key's kind tag is a divergence a compiler must reproduce
//!
//! There is no single SQL behaviour to match for text-versus-number: `DuckDB` casts and joins
//! `'1'` with `1`, `PostgreSQL` rejects `text = integer`. This model picks a third thing, no
//! cross-kind join and no error, as the only total engine-independent option -- so a compiler must
//! not emit a bare `l.k = r.k` across kinds.
//!
//! That rule is about each value's **runtime** kind, which a compiler cannot see: it has only
//! [`ColumnSchema::declared_kind`](super::catalog::ColumnSchema::declared_kind). Reproducing it in
//! SQL is therefore sound exactly when the catalog describes the kinds the source's values
//! actually have -- true of a statically typed engine, not of `SQLite`, which stores a type per
//! cell. A catalog precondition, not something a blueprint can be checked for, which is why
//! [`validate`](super::validate::validate) does not reject such a join.
...
impl Value {
    ...
    ///
    /// Used by [`Predicate::prepare`](super::predicate::Predicate::prepare) to coerce a
    ...
    ///   lenient cascade [`TimestampSource::parse`](super::expr::TimestampSource::parse) uses.
    ...
    ///
    /// The kind tag is a deliberate divergence from every engine, which a compiler has to
    /// reproduce rather than inherit -- see this module's docs.
    #[must_use]
    ...
    /// [`TimestampSource::parse`](super::expr::TimestampSource::parse) when the column is not
    /// already a `Timestamp`.
    ///
    ...
    /// different kinds returns `None`, as does any comparison involving `Null` -- including
    /// `Null` against `Null`, since SQL `NULL` is not equal to itself. A `None` result makes the
...
mod tests {
    ...
    /// `DuckDB` and `PostgreSQL` both give floats a *total* order for comparison: `NaN = NaN` is
    /// true and `-0.0 = 0.0` is true. Dropping those rows -- as treating `NaN` like `NULL` did,
    /// and as rendering `-0.0` as its own key did -- loses rows a compiled view keeps.
    #[test]
    ...
    /// `COALESCE(col <op> lit, FALSE)` -- run by an engine that gives floats the *same* total
    /// order `join_key_part` already commits to. `partial_cmp` answers `None` for any pair
