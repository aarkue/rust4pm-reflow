...
//!
//! Split three ways by what each part needs:
//!
//! - [`meta`] -- the `ocel-meta.json` manifest and the naming rules. Pure, always available.
//! - [`export`] -- writing a container (`ocel-bundle`, plus `ocel-bundle-parquet` for Parquet
//!   storage). Needs no connector: writing a CSV or Parquet file is not reading one.
//! - [`blueprint`] -- the
//!   [`Blueprint`](crate::core::event_data::object_centric::extraction::Blueprint) that turns a
//!   container's tables back into an OCEL (`extraction-blueprint`). *Running* it needs a row
//!   provider over the container's files, which `extraction-dbcon` supplies.
