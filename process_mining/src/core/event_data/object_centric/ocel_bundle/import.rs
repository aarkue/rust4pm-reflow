...
//!
//! Direct rather than via the extraction engine: the bundled layout is fixed and self-describing,
//! so there is nothing to join, filter or infer. That keeps `.ocel.zip` an ordinary
//! [`Importable`](crate::core::io::Importable) format alongside `.ocel.csv` and `.jsonocel`,
//! readable with no connector and on `wasm32`.
//!
//! (The [`Blueprint`](super::blueprint::blueprint_for) route reads the same layout through the
//! extraction engine instead. That exists for a different job -- reading a container *as a data
//! source* in the blueprint editor, and compiling a Parquet one to SQL views -- and shares this
//! module's [`meta`](super::meta) so the two cannot disagree about the layout.)
//!
//! # Memory
//!
//! A directory is read a table at a time, straight from disk. An archive is expanded into a
//! temporary directory first, one entry at a time: a Parquet file is read footer-first, and a ZIP
//! entry is only seekable when it was stored uncompressed -- which the format does not require
//! and a person handed a `.ocel.zip` cannot check. Peak memory stays flat either way. An archive
//! costs disk for its expanded size, freed when the [`Container`] drops.
...
impl Container {
    ...
    /// Open an archive already in memory, for contents with no path -- a browser upload, or bytes
    /// off a network. Holds every table in memory. Prefer [`Container::open`] where there is a
    /// path.
    ///
    /// # Errors
    /// See [`BundleImportError`].
...
fn read_csv<R: io::Read>(
    ...
    for record in rdr.records() {
        ...
        for (name, cell) in header.iter().zip(record.iter()) {
            ...
            // only reading available -- and it is why CSV storage cannot carry an attribute
