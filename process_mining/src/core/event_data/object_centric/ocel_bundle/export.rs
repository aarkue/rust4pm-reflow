...
//! every later observation, one row per changed attribute. So the split here is by timestamp --
//! observations at the epoch to one table, the rest to the other -- which is lossless in both
//! directions.
//!
//! An object whose earliest observation is *after* the epoch has no initial value, and its cell
//! is left empty rather than back-dating the first observation, which would move it in time.
...
fn write_archive<W: Write + std::io::Seek>(
    ...
    // `writer_properties` compresses each column chunk with ZSTD -- deflating the entry on top
...
fn build<O: ReadableOCEL + ?Sized>(
    ...
    // O(types x entities) -- on a log with dozens of types, dozens of complete passes. The tables
    // are built up front and every entity is routed to its own in a single pass instead.
