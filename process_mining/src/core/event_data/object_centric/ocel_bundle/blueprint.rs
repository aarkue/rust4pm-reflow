...
//! `Source` node per declared table, one mapping per thing that table produces. What this buys
//! over a hand-written reader is the rest of the extraction subsystem for free -- validation
//! against the discovered schema, a per-mapping [`ExtractionReport`](super::super::extraction::ExtractionReport),
//! and the SQL compiler, which turns a *Parquet* container into `DuckDB` views that never
//! materialise the log at all. (A CSV container cannot compile: every column is text, so the
//! timestamps and numeric attributes need the row executor's parsing.)
