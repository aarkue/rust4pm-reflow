...
// differ at a fixed position, so no type name can make two of these collide -- and neither can
// equal the bare `e2o`/`o2o`.
...
///
/// Import never needs this -- `ocel-meta.json` is authoritative and its declared paths are used
/// verbatim -- but an exporter does, and having both here keeps the two consistent.
#[must_use]
